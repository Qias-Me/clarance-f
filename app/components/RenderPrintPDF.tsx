...
const RenderPrintPDF: React.FC<FormProps> = ({ data, actionData }) => {
  ...
  const handleActionClick = async (action: string) => {
    ...
    if (action === "clientPDF") {
      ...
      console.log("🖥️ Please use the server-side PDF generation button for debugging.");
      setDownloadStatus("Client-side PDF generation disabled for development. Use server-side generation for debugging.");
      setLoading(false);
      return;

      // ORIGINAL CLIENT-SIDE CODE (commented out):
      // try {
