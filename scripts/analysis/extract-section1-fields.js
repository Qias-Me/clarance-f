...
try {
  ...
  console.log('Extracting fields for section 1...');
  const section1Fields = extractFieldsForSection(fieldHierarchy, 1);
  
  console.log(`Found ${section1Fields.length} fields in section 1:`);
  
  // Display field details
  section1Fields.forEach((field, index) => {
    console.log(`\nField ${index + 1}:`);
  ...
  const outputPath = 'scripts/analysis/section1-fields.json';
  fs.writeFileSync(outputPath, JSON.stringify(section1Fields, null, 2));
  console.log(`\nSection 1 fields saved to ${outputPath}`);
