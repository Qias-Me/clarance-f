 ...
 */
export async function optimizedCategorizeFields(
  engine: RuleEngine,
  fields: PDFField[]
): Promise<CategorizedField[]> {

  // Create a key based on fields content for memoization
  const createCacheKey = (fieldsArray: PDFField[]) => {
  ...
  const memoizedCategorize = memoize(
    (fieldsToProcess: PDFField[]) => engine.categorizeFields(fieldsToProcess),
    createCacheKey,
    'engineCategorize'
  );
 ...
 */
export async function batchCategorizeFields(
  engine: RuleEngine,
  fields: PDFField[],
  ...
  const results: CategorizedField[] = [];
