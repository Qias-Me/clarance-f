 ...
 */
function loadExistingSectionData(outputPath: string): Record<string, CategorizedField[]> | null {
 ...
 */
function applyEnhancedCategorization(
  ...
  for (const [pattern, fields] of Object.entries(patternGroups)) {
    ...
    for (const field of fields) {
      ...
      const result = enhancedMultiDimensionalCategorization(
        ...
        field.label,
        field.page,
        typeof field.value === 'string' ? field.value : getStringValue(field.value),
 ...
 */
function getReferenceCounts(referenceCounts: Record<number, number>): Record<number, number> {
 ...
 */
function getSectionSpecificThreshold(sectionId: number, defaultThreshold: number): number {
 ...
 */
function findFieldsByPatterns(fields: CategorizedField[], patterns: string[]): CategorizedField[] {
 ...
 */
function findFieldsWithPatterns(fields: CategorizedField[], patterns: string[]): CategorizedField[] {
  return fields.filter(field => {
    // Check field name against patterns
...
function applyAggressiveSpatialAnalysis(
  ...
  referenceCounts: Record<number, number>,
  problemSections: number[] = [2, 13, 16]
): {
  corrections: number;
  ...
  const sectionPageRanges: Record<number, number[]> = {
    ...
    if (remainingDeficit > 0) {
      ...
      if (remainingDeficit > 0) {
        ...
        if (expectedPages.length > 0) {
          ...
          const fieldsOnExpectedPages = (workingSectionFields['0'] || []).filter(
            field => field.page !== undefined && expectedPages.includes(field.page)
          );
...
async function applySelfHealingSteps(
  ...
  const workingSectionFields = JSON.parse(JSON.stringify(sectionFields)) as Record<string, CategorizedField[]>;
    ...
    if (excessCount > 0) {
      ...
      if (undersizedSections.includes(16)) {
        ...
        const fieldsToMove = section15Fields
          .filter((field: CategorizedField) => {
            // Low confidence fields
            if (field.confidence < 0.8) return true;
            
            // Reference-related fields - check label if it exists
            const name = field.name.toLowerCase();
            const label = field.label ? field.label.toLowerCase() : "";
            return ["reference", "know", "verify", "person", "friend"].some(keyword => 
              name.includes(keyword) || label.includes(keyword)
            );
          })
          .slice(0, moveCount);
      ...
      if (undersizedSections.includes(19)) {
        ...
        const fieldsToMove = section20Fields
          .filter(field => {
            ...
            const name = field.name.toLowerCase();
            const label = field.label ? field.label.toLowerCase() : "";
            return ["contact", "foreign", "national", "person"].some(keyword => 
              name.includes(keyword) || label.includes(keyword)
            );
          })
          .slice(0, moveCount);
      ...
      if (undersizedSections.includes(11)) {
        ...
        const fieldsToMove = section12Fields
          .filter(field => {
            ...
            const name = field.name.toLowerCase();
            const label = field.label ? field.label.toLowerCase() : "";
            return ["address", "residence", "lived", "city", "state", "zip"].some(keyword => 
              name.includes(keyword) || label.includes(keyword)
            );
          })
          .slice(0, moveCount);
  ...
  const spatialResult = applyAggressiveSpatialAnalysis(
    workingSectionFields,
    referenceCounts,
    [2, 13, 16] // Especially problematic sections based on analysis
  );
...
export async function runEnhancedSelfHealing(
  ...
  const workingSectionFields: Record<string, CategorizedField[]> = JSON.parse(JSON.stringify(sectionFields));
    ...
    if (workingSectionFields['0']?.length > 0) {
      ...
      if (unknownWithCoords.length > 0) {
        ...
        const underallocatedSections = deviationInfo.deviations
          .filter((d: any) => d.section > 0 && d.actual < d.expected)
          .map((d: any) => d.section);
        // For each unknown field with coordinates
        for (const field of unknownWithCoords) {
          let bestSection = 0;
          let bestScore = 0;
          let bestSectionFields: CategorizedField[] = [];
          ...
          if (bestSection > 0 && bestScore > 0.7) {
            ...
            // Add to new section
            if (!workingSectionFields[bestSection.toString()]) workingSectionFields[bestSection.toString()] = [];
