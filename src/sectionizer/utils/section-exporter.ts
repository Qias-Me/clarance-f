 ...
 */
export async function copyFinalizedRules(
  sourceDir: string = path.join(process.cwd(), 'src', 'sectionizer', 'rules'),
  targetDir: string = path.join(process.cwd(), 'src', 'rules')
): Promise<string[]> {
  ...
  try {
    ...
    const ruleFiles = files.filter(file => 
      file.match(/section\d+\.rules\.(ts|json)$/)
    );
 ...
 */
export async function exportAllFinalizedData(
  sectionFields: Record<string, CategorizedField[]>,
  sectionsOutputDir: string = 'src/finaloutputsections',
  rulesOutputDir: string = 'src/rules'
): Promise<{sections: string[], rules: string[]}> {
  // Export sections first
  const exportedSections = await exportFinalSections(sectionFields, sectionsOutputDir);
  
  // Then copy rules
  const exportedRules = await copyFinalizedRules(
    path.join(process.cwd(), 'src', 'sectionizer', 'rules'),
    rulesOutputDir
  );
