...
export class RuleLoader {
   ...
   */
  private async loadRulesFromTypeScript(section: number): Promise<SectionRuleSet | null> {
    try {
   ...
   * @returns Promise resolving to the rules for the section, or null if not found
   */
  private async loadRulesFromJson(section: number): Promise<SectionRuleSet | null> {
    try {
      ...
      const ruleFile = 
        path.join(this.rulesDir, `section${sectionStr}.rules.json`);
      ...
      if (Array.isArray(jsonData)) {
        ...
        return jsonData as SectionRuleSet;
      }
    } catch (error) {
   ...
   */
  private createDefaultRules(section: number): SectionRuleSet {
  ...
  public isFieldInSection(fieldName: string, section: number): boolean {
    if (!fieldName) return false;
    
    const lowerName = fieldName.toLowerCase();
    
    // Check if we have patterns for this section
    if (!sectionFieldPatterns[section as keyof typeof sectionFieldPatterns]) {
      return false;
    }
   ...
   */
  private convertJsonRulesToMatchRules(jsonRules: any[]): MatchRule[] {
   ...
   */
  private findPatternEndIndex(text: string): number {
   ...
   */
  public parseRulesFromTypeScriptExternal(rulesText: string, section: string): MatchRule[] {
