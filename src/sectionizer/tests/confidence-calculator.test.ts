...
import type { SectionOutput, EnhancedField } from '../types.js';

describe('ConfidenceCalculator', () => {
  // Helper function to create a mock EnhancedField object
  const createMockField = (id: string): EnhancedField => ({
    id: id,
    name: `field_${id}`,
    value: `value_${id}`,
    page: 1
  ...
  const createMockSectionOutput = (
    sectionNumber: string,
    matchedFields: number,
    fieldsData: Record<string, number>, // Number of fields to create in each subsection
    name?: string
  ): Record<string, SectionOutput> => {
    const fields: Record<string, EnhancedField[]> = {};
  ...
  describe('calculateConfidence', () => {
    it('should calculate confidence correctly for a section with fields', () => {
      const mockData = createMockSectionOutput(
        '1',
        8, // 8 matched fields
        {
          '_default': 4,
    ...
    it('should not override existing fieldsBySubsection if present', () => {
      ...
      const mockData: Record<string, SectionOutput> = {
        '4': {
          meta: {
            hasSubSections: false,
            confidence: 0,
