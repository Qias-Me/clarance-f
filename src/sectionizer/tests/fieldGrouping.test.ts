...
import type { SectionedField } from '../utils/fieldGrouping.js';
...
describe('Field Grouping Utils', () => {
  ...
  const testFields: SectionedField[] = [
  ...
  describe('groupFieldsBySection', () => {
    test('should group fields by section as record (default)', async () => {
      const result = await groupFieldsBySection(testFields) as Record<string, SectionedField[]>;
    ...
    test('should group fields by section as map', async () => {
      const result = await groupFieldsBySection(testFields, { returnType: 'map' }) as Map<number, SectionedField[]>;
    ...
    test('should handle deduplication correctly', async () => {
      ...
      const withDedup = await groupFieldsBySection(testFields) as Record<string, SectionedField[]>;
      expect(withDedup['1'].length).toBe(3); // Duplicate was filtered out
      
      // Without deduplication
      const noDedup = await groupFieldsBySection(testFields, { deduplicate: false }) as Record<string, SectionedField[]>;
    ...
    test('should return number keys if requested', async () => {
      const result = await groupFieldsBySection(testFields, { 
        returnType: 'record', 
        numberKeys: true 
      }) as Record<number, SectionedField[]>;
