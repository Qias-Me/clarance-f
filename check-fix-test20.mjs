...
console.log("🔍 Checking if the fix worked for field '9782 0 R' in test20...");

try {
  // Read the JSON file
  const data = fs.readFileSync('output/test20/categorized-fields.json', 'utf8');
  ...
  if (targetField) {
    ...
    } else if (isExplicitlyDetected && isCorrectSection) {
      console.log("\n🎉 MAJOR SUCCESS: Explicit detection working and field correctly categorized!");
      console.log("   (Field may have been moved by healing but ended up in correct section)");
    } else if (isExplicitlyDetected) {
      console.log("\n⚠️  PARTIAL SUCCESS: Explicit detection working but field in wrong section");
    } else {
      console.log("\n❌ FAILED: Explicit detection still not working");
    }
