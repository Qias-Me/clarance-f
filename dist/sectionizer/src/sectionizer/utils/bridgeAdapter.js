 ...
 */
import { enhancedMultiDimensionalCategorization, identifySectionByPage, enhancedSectionCategorization, updateFieldWithPageData, extractSectionInfoFromName, sectionClassifications, initPageCategorization as initPageCategorizationBridge, getNeighborFieldContext, refinedSectionPageRanges } from '../../../utilities/page-categorization-bridge.js';
import { extractSpatialInfo, calculateSpatialConfidenceBoost, getPositionalSectionScore } from './spatialAnalysis.js';
/**
 * Initialize page categorization system
 */
export async function initializePageCategorization() {
 ...
 */
export function applyPageCategorization(field) {
 ...
 * @returns Categorized field with section and confidence
 */
export function applyHeuristicRules(field) {
    // Initialize with default values
    const result = {
        ...field,
        section: 0,
    ...
    for (const sectionClassification of sectionClassifications) {
        if (!sectionClassification.fieldPathPatterns)
            continue;
        // Check if field name matches any pattern for this section
        const matchFound = sectionClassification.fieldPathPatterns.some(pattern => pattern.test(field.name));
 ...
 * @returns Categorized field with section and confidence
 */
export function advancedCategorization(field, neighborFields) {
    // Initialize with default values
    const result = {
        ...field,
        section: 0,
    ...
    try {
        ...
        if (spatialInfo) {
            ...
            const enhancedResult = enhancedMultiDimensionalCategorization(field.name, field.label || '', field.page || 0, fieldValueWithCoords, neighborFields || []);
            ...
            const enhancedResult = enhancedMultiDimensionalCategorization(field.name, field.label || '', field.page || 0, typeof field.value === 'string' ? field.value : undefined, neighborFields || []);
        ...
        if (result.confidence < 0.8) {
            ...
            if (heuristicResult && heuristicResult.section > 0 && heuristicResult.confidence > result.confidence) {
                ...
                if (heuristicResult.subsection)
                    result.subsection = heuristicResult.subsection;
                if (heuristicResult.entry)
                    result.entry = heuristicResult.entry;
            }
        }
        ...
        if (result.section === 0 || result.confidence < 0.5) {
            const fallbackResult = enhancedSectionCategorization(field.name, field.label || '', field.page || 0, field.value);
 ...
 */
export function getLimitedNeighborContext(fieldName, allFieldNames, contextSize = 2) {
    ...
    const beforeFields = allNeighbors.slice(Math.max(0, currentIndex - contextSize), currentIndex);
    const afterFields = allNeighbors.slice(currentIndex + 1, Math.min(allNeighbors.length, currentIndex + 1 + contextSize));
    // Return the limited context
    return [...beforeFields, fieldName, ...afterFields];
}
// Export utility functions directly from bridge
export { extractSectionInfoFromName, updateFieldWithPageData, getNeighborFieldContext, refinedSectionPageRanges, sectionClassifications };
