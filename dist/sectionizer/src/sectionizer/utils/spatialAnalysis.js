...
export function extractSpatialInfo(field) {
    ...
    const x = field.rect?.x || field.x || 0;
...
export function clusterFieldsSpatially(fields, maxDistance = 0.2) {
    ...
    Object.entries(fieldsByPage).forEach(([pageStr, pageFields]) => {
        ...
        const fieldsWithSpatialInfo = pageFields
            ...
            .filter(item => item.spatialInfo !== null);
        ...
        fieldsWithSpatialInfo.forEach(({ field, spatialInfo }) => {
            ...
            for (const cluster of clusters) {
                ...
                const distance = Math.sqrt(Math.pow(spatialInfo.relativeX - cluster.centroid.x, 2) +
            ...
            if (closestCluster !== null && minDistance <= maxDistance) {
                ...
                if (allSpatialInfos.length > 0) {
                    ...
                    allSpatialInfos.forEach(info => {
                        const distance = Math.sqrt(Math.pow(info.relativeX - closestCluster.centroid.x, 2) +
