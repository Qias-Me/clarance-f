...
export class RuleLoader {
    ...
    async loadRules(section) {
        ...
        try {
            ...
            if (strictSectionPatterns[section]) {
                const strictRules = strictSectionPatterns[section].map(pattern => ({
                    pattern,
                    subSection: '_default',
                    confidence: 0.99,
                ...
                if (strictRules.length > 0) {
                    ...
                    try {
                        ...
                        const strictRuleSet = { rules: strictRules, exclude: [] };
            ...
            const emptyRuleSet = { rules: [], exclude: [] };
     ...
     * @returns Object containing rules and exclusions
     */
    async loadRulesFromFile(section) {
    ...
    convertRules(jsonRules) {
        return jsonRules.map(rule => ({
            ...
            entryIndex: rule.entryIndexFn ?
                new Function('m', `return ${rule.entryIndexFn}`) :
                rule.entryPattern ?
                    ((m) => {
