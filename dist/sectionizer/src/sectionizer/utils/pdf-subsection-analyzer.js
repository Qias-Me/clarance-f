...
class SubsectionAnalyzer {
    ...
    verbose;
    constructor(options = {}) {
        this.outputDir = options.output || path.join(process.cwd(), 'validation-results');
     ...
     */
    initializeKnownSubsectionPatterns() {
    ...
    async initialize() {
        ...
        try {
            ...
            if (fs.existsSync(metadataPath)) {
                ...
                this.fieldMetadata = metadata.reduce((acc, field) => {
            ...
            if (fs.existsSync(fieldSectionsPath)) {
                ...
                for (const [fieldName, data] of Object.entries(sections)) {
                    if (this.fieldMetadata[fieldName]) {
                        this.fieldMetadata[fieldName].section = data.section;
                        this.fieldMetadata[fieldName].sectionName = data.sectionName;
                    }
                }
     ...
     */
    async analyzeFieldHierarchy(data) {
        ...
        try {
            ...
            if (typeof data === 'object' && data !== null) {
                ...
                for (const [pageNum, pageData] of Object.entries(data)) {
                    if (!pageData || typeof pageData !== 'object' || !('fieldsByValuePattern' in pageData))
                        continue;
                    ...
                    for (const [patternKey, patternData] of Object.entries(pageData.fieldsByValuePattern)) {
                        ...
                        for (const [regexKey, regexData] of Object.entries(patternData.fieldsByRegex || {})) {
     ...
     */
    determineSubsection(field, section) {
     ...
     */
    generateSubsectionReport(fields) {
     ...
     */
    extractPatternFromFieldName(fieldName) {
     ...
     */
    generateFieldSubsectionMapping(fields) {
        const mapping = {};
        for (const field of fields) {
            if (field.section && field.subsection) {
                mapping[field.name] = {
     ...
     */
    generateSubsectionCoverageReport(subsections) {
        // Group by section
        const sectionMap = {};
     ...
     */
    identifyMissingSubsections(subsections) {
     ...
     */
    suggestImprovements(fields, subsections) {
        ...
        if (fieldsWithoutSubsections.length > 0) {
            ...
            fs.writeFileSync(unassignedPath, JSON.stringify(fieldsWithoutSubsections.map(f => ({
                name: f.name,
                section: f.section,
                label: f.label
            })), null, 2));
     ...
     */
    async analyzeHierarchy(hierarchyPath) {
        try {
...
async function main() {
    try {
        // Use our centralized command line utility functions
        const program = configureUtilityCommandParser('PDF Field Subsection Analyzer Tool');
        // Add specific options for this utility
        program
            .requiredOption('-i, --input <path>', 'Path to field hierarchy JSON file')
            .option('-o, --output <path>', 'Output directory for results', path.join(process.cwd(), 'validation-results'))
            .option('-v, --verbose', 'Enable verbose logging', false);
        // Parse arguments
        const options = parseUtilityArgs(program);
        if (!options.input) {
        ...
        // Initialize analyzer with parsed options
        const analyzer = new SubsectionAnalyzer(options);
        await analyzer.initialize();
        // Process the hierarchy file
        await analyzer.analyzeHierarchy(options.input);
        console.log('Analysis complete');
    }
    catch (error) {
        console.error('Error in subsection analyzer:', error);
        process.exit(1);
    }
}
