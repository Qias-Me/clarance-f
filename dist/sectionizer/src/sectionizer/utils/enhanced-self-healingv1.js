...
import { calculateRelatedSections, expectedFieldCounts, sectionFieldPatterns, sectionPageRanges, sectionStructure, } from "./field-clusterer.js";
 ...
 */
function verifyFieldCountAlignment(sectionFields, referenceCounts, threshold = 0.1 // Use stricter threshold (changed from 0.7)
) {
    ...
    for (const [sectionKey, expectedCount] of Object.entries(referenceCounts)) {
        ...
        if (deviationPercentage <= threshold) {
            ...
            if (criticalSections.includes(sectionNumber) &&
    ...
    console.log(`\nAlignment check ${isGoodEnough ? "PASSED" : "FAILED"}: ${isGoodEnough
...
function allSectionsAligned(sectionFields, referenceCounts, deviationThreshold = 0.2) {
    ...
    for (const section of criticalSections) {
        if (!sectionFields[section.toString()] ||
 ...
 */
export function applyEnhancedCategorization(unknownFields, engine) {
    ...
    for (const [pattern, fields] of Object.entries(patternGroups)) {
        ...
        for (const field of fields) {
            ...
            const result = enhancedMultiDimensionalCategorization(field.name, field.label, field.page, typeof field.value === "string"
                ? field.value
                : getStringValue(field.value), [] // We don't have neighbor fields here
            );
 ...
 */
function findSectionFieldsByPatterns(unknownFields, sectionId, confidenceScore, pageRanges) {
    ...
    if (pageRanges && pageRanges.length > 0 && maxAdditionalFields > 0) {
        ...
        console.log(`Found ${candidatePageFields.length} potential page-based matches, taking ${pageBasedMatches.length} for section ${sectionId} (max allowed: ${maxAdditionalFields})`);
    }
    // Combine pattern matches and limited page-based matches
    const allMatches = [...exactMatches, ...pageBasedMatches];
    console.log(`Total matches for section ${sectionId}: ${allMatches.length} (${exactMatches.length} pattern + ${pageBasedMatches.length} page-based)`);
 ...
 * @returns Array of fields that match the section patterns
 */
function findSectionFieldsByPatternsBackup(unknownFields, sectionId, confidenceScore = 0.9, pageRanges) {
    // Get patterns for the requested section
    const sectionPatterns = sectionFieldPatterns[sectionId];
    if (!sectionPatterns || sectionPatterns.length === 0) {
        console.log(`No patterns defined for section ${sectionId}`);
 ...
 */
function applyAggressiveSpatialAnalysis(workingSectionFields, referenceCounts, problemSections) {
    let corrections = 0;
    // Get unknown fields
...
async function applySelfHealingSteps(ruleEngine, sectionFields, referenceCounts, deviationThreshold) {
    ...
    console.log(`Processing undersized sections: ${undersizedSections.join(", ")}`);
    ...
    if (shouldProcessUnknownFields || missingSections.length > 0) {
        ...
        console.log(`Processing ${unknownFields.length} unknown fields (expected: ${expectedUnknownCount})`);
        ...
        sectionsToFix.sort((a, b) => {
            ...
            if (aPageRange && bPageRange) {
                const aPages = Array.isArray(aPageRange) ? aPageRange : [aPageRange];
                const bPages = Array.isArray(bPageRange) ? bPageRange : [bPageRange];
                // If both sections include page 6, prioritize section 8
                if (aPages.includes(6) && bPages.includes(6)) {
                    if (a === 8 && b === 7)
                        return -1; // Section 8 before section 7
                    if (a === 7 && b === 8)
                        return 1; // Section 8 before section 7
                }
            }
        ...
        for (const sectionId of sectionsToFix) {
            console.log(`Fixing section ${sectionId} (${sectionStructure[sectionId] || sectionStructure[0]})`);
            ...
            const deficit = expectedCount - currentCount;
            if (deficit <= 0) {
                console.log(`  Section ${sectionId} has enough fields (${currentCount}/${expectedCount})`);
                continue;
            }
            console.log(`  Section ${sectionId} needs ${deficit} more fields (has ${currentCount}, needs ${expectedCount})`);
            // First try: Pattern-based field matching
            // applyAggressiveSpatialAnalysis(workingSectionFields, referenceCounts, sectionsToFix);
            // Use the generalized pattern-based field finder
            const patternFields = findSectionFieldsByPatterns(unknownFields, sectionId, 0.95, // High confidence score
            sectionPageRanges[sectionId] // Page ranges for this section
            );
            // const patternFields = await ruleEngine.categorizeFields(unknownFields)
            if (patternFields.length > 0) {
                console.log(`  Found ${patternFields.length} fields for section ${sectionId} using pattern matching`);
                // Remove these fields from unknown
                const fieldIds = new Set(patternFields.map((f) => f.id));
                const newUnknownFields = unknownFields.filter((f) => !fieldIds.has(f.id));
            ...
            const remainingDeficit = expectedCount - workingSectionFields[sectionId.toString()].length;
            if (remainingDeficit > 0 && sectionPageRanges[sectionId]) {
                const [minPage, maxPage] = sectionPageRanges[sectionId];
                console.log(`  Searching for section ${sectionId} fields on pages ${minPage}-${maxPage}`);
                // Find fields on the pages for this section
                const pageBasedFields = unknownFields
                    .filter((field) => field.page && field.page >= minPage && field.page <= maxPage)
                    .slice(0, remainingDeficit); // Only take what we need
                if (pageBasedFields.length > 0) {
                    console.log(`  Found ${pageBasedFields.length} fields for section ${sectionId} based on page range`);
        ...
        console.log(`Skipping unknown field processing: ${unknownFields.length} fields (expected: ${expectedUnknownCount})`);
    }
    ...
    for (const sourceSection of oversizedSections) {
        ...
        if (excess <= 0)
            continue; // Skip if not actually oversized
        console.log(`Fixing oversized section ${sourceSection} (${sectionStructure[sourceSection] || "Unknown"}): ${actualCount} fields vs expected ${expectedCount}`);
        // Get potential destination sections (related + undersized)
        const potentialDestinations = (relatedSections[sourceSection] || []).filter((section) => undersizedSections.includes(section));
        if (potentialDestinations.length === 0) {
            console.log(`  No related undersized sections found for section ${sourceSection}`);
            continue;
        }
        console.log(`  Potential destination sections: ${potentialDestinations.join(", ")}`);
        // Process each potential destination
        for (const destSection of potentialDestinations) {
            if (excess <= 0)
                break; // Stop if we've moved enough fields
            ...
            if (destDeficit <= 0)
                continue; // Skip if destination doesn't need more fields
            const moveCount = Math.min(excess, destDeficit);
            console.log(`  Section ${destSection} needs ${destDeficit} more fields. Moving up to ${moveCount} fields from section ${sourceSection}`);
            ...
            const fieldsToMove = sourceFields
                ...
                .slice(0, moveCount);
            console.log(`  Selected ${fieldsToMove.length} fields to move from section ${sourceSection} to ${destSection}`);
            // Move the fields
            let movedCount = 0;
            ...
            console.log(`  Moved ${movedCount} fields from section ${sourceSection} to ${destSection}`);
            // Update excess count
            excess -= movedCount;
        }
    }
 ...
 */
function getKeywordMatchScore(field, keywords) {
 ...
 */
export async function runEnhancedSelfHealing(ruleEngine, sectionFields, referenceCounts, outputPath, deviationThreshold = 0.2) {
    ...
    const initialAligned = verifyFieldCountAlignment(workingSectionFields, simplifiedCounts, deviationThreshold);
    ...
    while ((!aligned || workingSectionFields["0"]?.length > 0) && // Continue if there are still unknown fields
        iterations < maxIterations) {
        iterations++;
        console.log(`\n🔄 Self-healing iteration ${iterations}...`);
        // Apply various self-healing techniques
        const result = await applySelfHealingSteps(ruleEngine, workingSectionFields, simplifiedCounts, deviationThreshold);
        ...
        aligned = verifyFieldCountAlignment(workingSectionFields, simplifiedCounts, deviationThreshold);
