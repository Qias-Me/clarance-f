...
export class EnhancedSelfHealer {
    ...
    static existingSectionData = null;
     ...
     */
    verifyFieldCountAlignment(sectionFields, referenceCounts, threshold = 0.3 // Use stricter threshold (changed from 0.7)
    ) {
     ...
     */
    allSectionsAligned(sectionFields, referenceCounts, deviationThreshold = 0.2) {
        // Get deviation info
     ...
     */
    loadExistingSectionData(outputPath) {
     ...
     */
    saveSectionData(sectionFields, outputPath) {
        try {
     ...
     */
    evaluateSectionDeviations(sectionFields, referenceCounts, deviationThreshold = 0.2) {
     ...
     */
    getSectionSpecificThreshold(sectionId, defaultThreshold) {
     ...
     */
    getDeviationInfo(sectionFields, referenceCounts) {
        const deviations = [];
        // Check each section with a reference count
        for (const [sectionKey, expectedCount] of Object.entries(referenceCounts)) {
            const sectionNumber = parseInt(sectionKey, 10);
     ...
     */
    getPotentialSectionFromPattern(pattern) {
     ...
     */
    ensureCriticalSections(sectionFields, criticalSections = this.criticalSections) {
     ...
     */
    criticalSectionsExist(sectionFields, criticalSections = this.criticalSections) {
        for (const section of criticalSections) {
            const sectionKey = section.toString();
     ...
     */
    trimOversizedSections(sectionFields, referenceCounts) {
