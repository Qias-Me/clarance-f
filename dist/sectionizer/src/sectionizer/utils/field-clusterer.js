...
export class FieldClusterer {
     ...
     */
    addToPatternMap(map, pattern, fieldName) {
        if (!map.has(pattern)) {
            map.set(pattern, []);
        }
     ...
     */
    groupFieldsByPrimaryPattern(fields, patterns, options) {
        const clusters = [];
     ...
     */
    refineClusters(clusters, options) {
        const result = [];
     ...
     */
    subdivideCluster(cluster, options) {
        const result = [];
     ...
     */
    calculateNamingConsistency(fields) {
        if (fields.length < 2)
            return 0.5;
    ...
    calculatePageConsistency(fields) {
        ...
        const pageInSection = (page, section) => {
     ...
     */
    calculateSectionPatternMatch(fields) {
     ...
     */
    suggestSectionForCluster(cluster) {
