 ...
 */
export async function updateRules(sectionFields, referenceCounts, outputPath = 'section-data') {
    ...
    for (const deviation of problemSections.filter(d => d.deviation > 0)) {
        ...
        sectionFieldsForSection.sort((a, b) => a.confidence - b.confidence);
        ...
        const patterns = findCommonPatterns(potentialMisclassified.map((f) => f.name));
    ...
    for (const deviation of problemSections.filter(d => d.deviation < 0)) {
        ...
        const potentialMatches = unknownFields.filter((f) => sectionPattern.test(f.name) ||
        ...
        if (potentialMatches.length > 0) {
            ...
            const patterns = findCommonPatterns(potentialMatches.map((f) => f.name));
            if (patterns.length > 0) {
        ...
        for (const otherSection of oversizedSections) {
            ...
            const potentialMisclassified = otherSectionFields.filter((f) => f.confidence < 0.85 && (sectionPattern.test(f.name) ||
                (f.label && sectionPattern.test(f.label))));
    ...
    await fs.promises.writeFile(rulesOutputPath, JSON.stringify(rulesUpdates, null, 2));
 ...
 */
function applyRulesToFields(sectionFields, rulesUpdates) {
 ...
 */
function findCommonPatterns(fieldNames) {
