...
import { sectionFieldPatterns, sectionKeywords, sectionPageRanges, expectedFieldCounts, subsectionPatterns, entryPatterns, sectionEntryPrefixes } from './field-clusterer.js';
...
export class ConsolidatedSelfHealingManager {
    ...
    iteration = 0;
    previousUnknownCount = Infinity; // Note: This seems unused in the main runSelfHealing loop currently
    previousMiscategorizedCount = Infinity;
    previousSectionZeroCount = Infinity; // Added for tracking Section 0
    /**
     ...
     */
    constructor(maxIterations = 15, deviationThreshold = 0.1) {
     ...
     */
    async runSelfHealing(ruleEngine, sectionFields, referenceCounts, outputPath) {
        ...
        const initialDeviations = this.calculateDeviations(workingSectionFields, referenceCounts, this.deviationThreshold);
        ...
        while (this.iteration < this.maxIterations) {
            ...
            const stepResult = await this.applySelfHealingStep(ruleEngine, workingSectionFields, referenceCounts);
            // Update the result data
            result.corrections += stepResult.corrections;
            if (stepResult.corrections > 0) {
                helpers.logWithTimestamp(`Made ${stepResult.corrections} corrections in iteration ${this.iteration} (applySelfHealingStep)`, chalk.green);
                ...
                helpers.logWithTimestamp(`No corrections made in iteration ${this.iteration} by applySelfHealingStep`, chalk.yellow);
            }
            // Check if we've achieved acceptable alignment
            const currentDeviations = this.calculateDeviations(workingSectionFields, referenceCounts, this.deviationThreshold);
            ...
            helpers.logWithTimestamp(`Iter ${this.iteration}: Align: ${currentAlignmentScore.toFixed(2)}%, Miscateg: ${miscategorizedCount}, S0: ${currentSectionZeroCount}, Perfect: ${isPerfectlyBalanced ? 'YES' : 'NO'}, Step Corrections: ${stepResult.corrections}`, chalk.cyan);
            if (currentSectionZeroCount === 0 && isPerfectlyBalanced) {
                helpers.logWithTimestamp(`🎯 PERFECT BALANCE ACHIEVED! Section 0 empty and ALL sections at expected counts!`, chalk.green);
                result.success = true;
                break;
            }
            ...
            if (currentSectionZeroCount === 0 && !isPerfectlyBalanced) {
                ...
                if (currentAlignmentScore >= 99.5) {
                    helpers.logWithTimestamp(`Section 0 empty and alignment score ${currentAlignmentScore.toFixed(2)}%, but not perfectly balanced. Continuing...`, chalk.cyan);
                }
                else {
                    helpers.logWithTimestamp(`Section 0 empty but alignment score only ${currentAlignmentScore.toFixed(2)}%. Continuing to improve alignment.`, chalk.cyan);
                }
            }
            ...
            if (this.iteration > 1 && shouldStopForStall) {
                helpers.logWithTimestamp(`Stalled at iteration ${this.iteration}. Score: ${currentAlignmentScore.toFixed(2)}%, Miscategorized: ${miscategorizedCount}, S0: ${currentSectionZeroCount}`, chalk.yellow);
                // Try aggressive section 0 processing first if it has fields
                if (currentSectionZeroCount > 0) {
                    helpers.logWithTimestamp(`Applying aggressive Section 0 processing (${currentSectionZeroCount} fields)...`, chalk.magenta);
                    const sectionZeroResult = this.aggressivelyProcessSectionZero(workingSectionFields, ruleEngine, referenceCounts);
                    result.corrections += sectionZeroResult.corrections;
                    if (sectionZeroResult.corrections > 0) {
                        helpers.logWithTimestamp(`Aggressive Section 0 processing made ${sectionZeroResult.corrections} corrections`, chalk.green);
                        ...
                        helpers.logWithTimestamp(`State after Section 0 processing: Miscateg: ${this.previousMiscategorizedCount}, S0: ${this.previousSectionZeroCount}. Continuing loop.`, chalk.magenta);
                        continue;
                    }
                }
                ...
                if (!didApplyLastResort && (currentSectionZeroCount > 0 || miscategorizedCount > 0)) {
                    helpers.logWithTimestamp(`Applying direct section balance as last resort (S0: ${currentSectionZeroCount}, Miscateg: ${miscategorizedCount})...`, chalk.red);
                    const directResult = this.directSectionBalance(workingSectionFields, // Pass current working copy
                    referenceCounts);
                    result.corrections += directResult.corrections;
                    didApplyLastResort = true;
                    if (directResult.corrections > 0) {
                        helpers.logWithTimestamp(`Direct balance made ${directResult.corrections} corrections`, chalk.green);
                        ...
                        helpers.logWithTimestamp(`State after direct balance: Miscateg: ${this.previousMiscategorizedCount}, S0: ${this.previousSectionZeroCount}. Continuing loop.`, chalk.magenta);
                        continue; // Continue to the next iteration to see if applySelfHealingStep can refine
                    }
                    else {
                        helpers.logWithTimestamp(`Direct balance found no further corrections. Stopping due to stall.`, chalk.yellow);
            ...
            if (currentSectionZeroCount > 0 && this.iteration >= this.maxIterations * 0.9) {
                helpers.logWithTimestamp(`Near max iterations with ${currentSectionZeroCount} fields still in Section 0. Applying final aggressive processing...`, chalk.red);
                ...
                if (finalResult.corrections > 0) {
                    helpers.logWithTimestamp(`Final aggressive processing moved ${finalResult.corrections} fields from Section 0`, chalk.green);
                    workingSectionFields = this.deduplicateFieldsById(workingSectionFields);
                    result.finalSectionFields = helpers.deepClone(workingSectionFields);
            ...
            if (currentSectionZeroCount === 0 && currentAlignmentScore < 95 && this.iteration >= this.maxIterations * 0.3) {
                helpers.logWithTimestamp(`Section 0 empty but alignment score ${currentAlignmentScore.toFixed(2)}%. Applying intelligent rebalancing...`, chalk.magenta);
                ...
                if (rebalanceResult.corrections > 0) {
                    helpers.logWithTimestamp(`Intelligent rebalancing moved ${rebalanceResult.corrections} fields`, chalk.green);
                    workingSectionFields = this.deduplicateFieldsById(workingSectionFields);
                    result.finalSectionFields = helpers.deepClone(workingSectionFields);
                    // Update counts for next iteration
            ...
            if (currentSectionZeroCount === 0 && currentAlignmentScore < 95 && this.iteration >= this.maxIterations * 0.6) {
                helpers.logWithTimestamp(`Applying final aggressive rebalancing for severely imbalanced sections...`, chalk.red);
                ...
                if (finalRebalanceResult.corrections > 0) {
                    helpers.logWithTimestamp(`Final aggressive rebalancing moved ${finalRebalanceResult.corrections} fields`, chalk.green);
                    workingSectionFields = this.deduplicateFieldsById(workingSectionFields);
                    result.finalSectionFields = helpers.deepClone(workingSectionFields);
                    // Update counts for next iteration
        ...
        const finalDeviations = this.calculateDeviations(workingSectionFields, referenceCounts, this.deviationThreshold);
    ...
    async processUnknownFields(ruleEngine, unknownFields, autoUpdate = true, logProgress = true) {
        ...
        while (currentUnknownFields.length > 0 && iteration < this.maxIterations) {
            ...
            if (enhancedResult.corrections > 0) {
                ...
                currentUnknownFields = currentUnknownFields.filter(field => !enhancedResult.recategorized.some(r => r.id === field.id));
            }
     ...
     */
    aggressivelyProcessSectionZero(sectionFields, _ruleEngine, referenceCounts) {
        ...
        let corrections = 0;
        const fieldsToMove = [];
        helpers.logWithTimestamp(`Processing ${sectionZeroFields.length} fields in Section 0 with comprehensive analysis...`, chalk.blue);
        for (const field of sectionZeroFields) {
            ...
            if (targetSection > 0) {
                ...
                if (isValidAssignment) {
                    ...
                    helpers.logWithTimestamp(`${reason}: "${field.name}" -> Section ${targetSection} (confidence: ${confidence.toFixed(2)})`, chalk.green);
                }
                else {
                    helpers.logWithTimestamp(`Rejected assignment: "${field.name}" -> Section ${targetSection} (section oversized)`, chalk.yellow);
                }
            }
        }
        ...
        helpers.logWithTimestamp(`Aggressive Section 0 processing completed: ${corrections} fields moved`, corrections > 0 ? chalk.green : chalk.yellow);
        return { corrections };
    }
    /**
     * Analyze complex field patterns that might not be caught by standard parsing
     */
    analyzeComplexFieldPattern(field) {
     ...
     */
    analyzeSpatialProximity(field, sectionFields) {
        ...
        for (const [sectionKey, fields] of Object.entries(sectionFields)) {
            ...
            if (section === 0)
                continue; // Skip uncategorized fields
            for (const otherField of fields) {
                if (otherField.page === field.page && otherField.rect) {
                    const distance = Math.sqrt(Math.pow(fieldX - otherField.rect.x, 2) + Math.pow(fieldY - otherField.rect.y, 2));
     ...
     */
    comprehensiveFieldCategorization(field) {
     ...
     */
    analyzeFieldContentForSection(field) {
        const name = field.name.toLowerCase();
        const value = field.value?.toString().toLowerCase() || '';
     ...
     */
    fallbackFieldCategorization(field, sectionFields, referenceCounts) {
        const name = field.name.toLowerCase();
        const page = field.page || 0;
        const value = field.value?.toString().toLowerCase() || '';
     ...
     */
    getPageBasedSection(page) {
        // Page ranges for different sections
        if (page >= 17 && page <= 33)
            return 13; // Employment
        if (page >= 14 && page <= 16)
            return 12; // Education
        if (page >= 10 && page <= 13)
            return 11; // Residence
        if (page >= 39 && page <= 44)
            return 17; // Relationships
        if (page >= 45 && page <= 62)
            return 18; // Relatives
        if (page >= 63 && page <= 66)
            return 19; // Foreign contacts
        if (page >= 67 && page <= 87)
            return 20; // Foreign activities
        if (page >= 88 && page <= 103)
            return 21; // Foreign business
        if (page >= 5 && page <= 9)
     ...
     */
    getContentBasedSection(name, value) {
     ...
     */
    analyzeKeywordsWithContext(field) {
        const name = field.name.toLowerCase();
        const value = field.value?.toString().toLowerCase() || '';
    ...
    isEmploymentField(field) {
        ...
        const hasEmploymentStructure = name.includes('section13') ||
            (isEmploymentPage && (name.includes('textfield') ||
    ...
    analyzeDateFieldContext(field) {
        ...
        if (page >= 17 && page <= 33)
            return 13; // Employment pages
        if (page >= 14 && page <= 16)
            return 12; // Education pages
        if (page >= 10 && page <= 13)
            return 11; // Residence pages
        if (page >= 39 && page <= 44)
            return 17; // Relationship pages
        return 0; // No specific context found
    }
    /**
     * Check if all sections are perfectly balanced (at their expected counts)
     */
    isPerfectlyBalanced(sectionFields, referenceCounts) {
     ...
     */
    logDetailedBalanceReport(sectionFields, referenceCounts) {
        helpers.logWithTimestamp('📊 Detailed Balance Report:', chalk.blue);
        const imbalancedSections = [];
        for (const [sectionStr, expectedData] of Object.entries(referenceCounts)) {
            const section = parseInt(sectionStr);
            const currentCount = (sectionFields[section.toString()] || []).length;
            const expectedCount = expectedData.fields;
     ...
     */
    analyzeFieldContext(field, sectionFields) {
        const name = field.name.toLowerCase();
     ...
     */
    validatePageBasedAssignment(field, proposedSection) {
     ...
     */
    validateSectionAssignment(targetSection, sectionFields, referenceCounts) {
        ...
        const getOversizeThreshold = (section) => {
            switch (section) {
     ...
     */
    intelligentFieldRebalancing(sectionFields, referenceCounts) {
        let corrections = 0;
        const fieldsToMove = [];
        ...
        for (const [sectionStr, expectedData] of Object.entries(referenceCounts)) {
            ...
            if (ratio > 1.05)
                status = 'oversized'; // Lowered from 1.1 to 1.05
            else if (ratio < 0.95)
                status = 'undersized'; // Raised from 0.9 to 0.95
        ...
        if (section1Analysis && section1Analysis.status === 'oversized') {
            ...
            for (const candidate of misplacedFields) {
                ...
                if (targetSectionAnalysis && targetSectionAnalysis.status !== 'oversized') {
                    ...
                    helpers.logWithTimestamp(`Section 1 rebalancing: "${candidate.field.name}" -> Section ${candidate.suggestedSection} (${candidate.confidence.toFixed(2)})`, chalk.green);
                }
                else {
                    helpers.logWithTimestamp(`Skipping "${candidate.field.name}" -> Section ${candidate.suggestedSection} (target oversized or not found)`, chalk.yellow);
                }
            }
        }
            ...
            for (const candidate of candidateFields) {
                ...
                if (targetSectionAnalysis && targetSectionAnalysis.status !== 'oversized') {
                    ...
                    if (expectedTargetCount === 0 || currentTargetCount < expectedTargetCount * 1.2) {
                        ...
                        helpers.logWithTimestamp(`Rebalancing: "${candidate.field.name}" from Section ${oversized.section} -> Section ${candidate.suggestedSection} (${candidate.confidence.toFixed(2)})`, chalk.green);
                    }
                    else {
                        helpers.logWithTimestamp(`Skipping "${candidate.field.name}" -> Section ${candidate.suggestedSection} (target would be oversized: ${currentTargetCount}/${expectedTargetCount})`, chalk.yellow);
                    }
                }
                else {
                    helpers.logWithTimestamp(`Skipping "${candidate.field.name}" -> Section ${candidate.suggestedSection} (target oversized or not found)`, chalk.yellow);
     ...
     */
    aggressiveFieldRebalancing(sectionFields, referenceCounts) {
        let corrections = 0;
        const fieldsToMove = [];
        ...
        if (section13Deficit > 0) {
            ...
            for (const candidate of fieldsToMoveToSection13) {
                ...
                helpers.logWithTimestamp(`Employment recovery: "${candidate.field.name}" from Section ${candidate.fromSection} -> Section 13 (${candidate.confidence.toFixed(2)})`, chalk.red);
            }
        }
        else {
        ...
        if (section4Excess > 0) {
            ...
            for (const field of nonSSNFields) {
                ...
                helpers.logWithTimestamp(`SSN section cleanup: "${field.name}" from Section 4 -> Section ${newSection}`, chalk.yellow);
            }
        }
        ...
        if (section18Excess > 0) {
            ...
            for (const field of nonRelativeFields) {
                ...
                helpers.logWithTimestamp(`Relatives section cleanup: "${field.name}" from Section 18 -> Section ${newSection}`, chalk.yellow);
            }
        }
        ...
        if (section14Excess > 0) {
            ...
            for (const field of excessFields) {
                ...
                helpers.logWithTimestamp(`Section 14 cleanup: "${field.name}" from Section 14 -> Section ${newSection}`, chalk.yellow);
            }
        }
        // Log summary before executing moves
     ...
     */
    getBestAlternativeSection(field) {
        const page = field.page || 0;
        const name = field.name.toLowerCase();
        ...
        if (page >= 39 && page <= 44)
            return 17; // Relationship pages
        if (page >= 45 && page <= 62)
            return 18; // Relatives pages
        if (page >= 63 && page <= 66)
            return 19; // Foreign contacts pages
        if (page >= 67 && page <= 87)
            return 20; // Foreign activities pages
        // Field type based fallback
        if (name.includes('textfield'))
            return 11; // Most text fields are addresses
        if (name.includes('radiobutton'))
            return 9; // Most radio buttons are citizenship
        if (name.includes('dropdown'))
            return 11; // Most dropdowns are addresses
        // Default fallback
    ...
    hasEmploymentKeywords(field) {
        ...
        return employmentKeywords.some(keyword => name.includes(keyword) || value.includes(keyword));
    }
    /**
     * Check if field could potentially be an employment field based on context
     */
    couldBeEmploymentField(field, currentSection, sectionFields, referenceCounts) {
        ...
        const hasEmploymentContext = name.includes('section13') || // Explicitly in employment section
     ...
     */
    isDefinitelyRelativeField(field) {
        const name = field.name.toLowerCase();
     ...
     */
    getFieldsOnPage(page, sectionFields) {
        const pageFields = [];
     ...
     */
    applyEnhancedCategorization(unknownFields, _engine) {
        const recategorized = [];
     ...
     */
    async applySelfHealingStep(ruleEngine, sectionFields, referenceCounts) {
        ...
        helpers.logWithTimestamp(`Found ${oversizedSections.length} oversized and ${undersizedSections.length} undersized sections initially`, chalk.blue);
        ...
        if (oversizedSections.length === 0 && sectionZeroFields.length > 0 && undersizedSections.length > 0) {
            helpers.logWithTimestamp(`All target sections undersized. Using section 0 as the sole source with ${sectionZeroFields.length} fields.`, chalk.magenta);
            oversizedSections = [{
            ...
            helpers.logWithTimestamp(`Updated oversizedSections to use section 0. Now: ${oversizedSections.length} oversized section(s).`, chalk.magenta);
        }
        // Log current state of oversized and undersized sections after potential modification
        if (oversizedSections.length > 0) {
            helpers.logWithTimestamp(`Oversized sections for processing: ${oversizedSections.map(s => `${s.section} (+${s.deviation})`).join(', ')}`, chalk.yellow);
        }
        if (undersizedSections.length > 0) {
            helpers.logWithTimestamp(`Undersized sections for processing: ${undersizedSections.map(s => `${s.section} (-${Math.abs(s.deviation)})`).join(', ')}`, chalk.yellow);
        }
        ...
        if (oversizedSections.length === 0 || undersizedSections.length === 0) {
            helpers.logWithTimestamp(`No basis for corrections: Oversized count ${oversizedSections.length}, Undersized count ${undersizedSections.length}. Returning.`, chalk.yellow);
            return {
        ...
        for (const undersizedSection of undersizedSections) {
            ...
            const neededCount = Math.min(Math.abs(Math.ceil(undersizedSection.deviation)), Math.max(50, Math.abs(Math.ceil(undersizedSection.deviation * 0.8))) // Move at most 80% of needed fields per iteration
            );
            helpers.logWithTimestamp(`Section ${targetSection} needs ${neededCount} more fields`, chalk.blue);
            // Find candidate fields from oversized sections
            const candidatesFromOversized = this.findCandidatesForTargetSection(workingSectionFields, oversizedSections, targetSection, 
            ...
            if (candidatesFromOversized.length > 0) {
                helpers.logWithTimestamp(`Found ${candidatesFromOversized.length} candidates to move to section ${targetSection}`, chalk.green);
                ...
                this.moveFieldsToSection(workingSectionFields, fieldsToMove, targetSection, true // Force move even if confidence is high
                );
                totalCorrections += fieldsToMove.length;
                // Generate rule candidates if enough fields were moved
                if (fieldsToMove.length >= 3) {
                    const sectionRuleCandidates = this.extractRuleCandidates(fieldsToMove, targetSection);
                    if (sectionRuleCandidates.length > 0) {
                        // Store rule candidates for this section
                        ruleCandidates.set(targetSection.toString(), (ruleCandidates.get(targetSection.toString()) || []).concat(sectionRuleCandidates));
                        rulesGenerated += sectionRuleCandidates.length;
                        helpers.logWithTimestamp(`Generated ${sectionRuleCandidates.length} rule candidates for section ${targetSection}`, chalk.green);
                    }
                }
            }
            else {
                helpers.logWithTimestamp(`Could not find suitable candidates for section ${targetSection}`, chalk.yellow);
                ...
                if (oversizedSections.length > 0) {
                    ...
                    if (lowestConfidenceFields.length > 0) {
                        helpers.logWithTimestamp(`Falling back to moving ${lowestConfidenceFields.length} low-confidence fields from section ${sourceSection} to ${targetSection}`, chalk.yellow);
                        this.moveFieldsToSection(workingSectionFields, lowestConfidenceFields, targetSection, true // Force move even if confidence is high
                        );
        ...
        if (rulesGenerated > 0 && ruleCandidates.size > 0) {
            try {
                ...
                let validRuleCandidates = new Map();
        ...
        // Calculate final alignment score
        const finalDeviations = this.calculateDeviations(finalSectionFields, referenceCounts, this.deviationThreshold);
        const finalAlignmentScore = this.calculateAlignmentScore(finalDeviations);
        const alignmentImprovement = finalAlignmentScore - initialAlignmentScore;
        helpers.logWithTimestamp(`Moved ${totalCorrections} fields between sections, alignment improved by ${alignmentImprovement.toFixed(2)}%`, totalCorrections > 0 ? chalk.green : chalk.yellow);
        return {
     ...
     */
    extractRuleCandidates(fields, targetSection) {
     ...
     */
    findCandidatesForTargetSection(sectionFields, oversizedSections, targetSection, maxNeeded) {
        const candidates = [];
        ...
        for (const oversizedSection of oversizedSections) {
            ...
            const availableToTake = Math.min(Math.ceil(oversizedSection.deviation * 0.5), // Take at most half of the oversized amount per iteration
            maxNeeded - candidates.length, 50 // Cap at 50 fields per section per iteration
            );
            if (availableToTake <= 0)
                continue;
            // Log more information about the section we're trying to take from
            helpers.logWithTimestamp(`Looking for ${availableToTake} candidates in section ${sourceSection} (${sourceFields.length} fields)`, chalk.blue);
            // Find the most relevant fields for the target section
            const scoredFields = this.improvedScoreFieldsForTargetSection(sourceFields, targetSection, sourceSection);
            ...
            if (bestCandidates.length > 0) {
                ...
                helpers.logWithTimestamp(`Found ${bestCandidates.length} candidate fields from section ${sourceSection} for section ${targetSection} (scores: ${scoreInfo})`, chalk.blue);
            }
        ...
        if (candidates.length < maxNeeded * 0.5 && this.iteration >= Math.floor(this.maxIterations / 2)) {
            helpers.logWithTimestamp(`Not enough candidates found (${candidates.length}/${maxNeeded}). Trying more aggressive approach in iteration ${this.iteration}.`, chalk.yellow);
            ...
            for (const oversizedSection of oversizedSections) {
                ...
                if (candidates.findIndex(c => c.section === sourceSection) > maxNeeded / 2)
                    continue;
                const sourceFields = sectionFields[sourceSection.toString()] || [];
                // Find the lowest confidence fields regardless of their content relevance
                const lowestConfidenceFields = this.findLowConfidenceFields(sourceFields, Math.min(maxNeeded - candidates.length, 5));
     ...
     */
    improvedScoreFieldsForTargetSection(fields, targetSection, sourceSection // Added sourceSection to the signature if not already present
    ) {
        const scoredFields = [];
        for (const field of fields) {
            let score = 0;
     ...
     */
    findLowConfidenceFields(fields, count) {
        return [...fields]
     ...
     */
    moveFieldsToSection(sectionFields, fieldsToMove, targetSection, forceMove = false) {
        // Process each field
     ...
     */
    generateRuleCandidates(fields, targetSection) {
        try {
                    ...
                    if (patternType && patternValue) {
                        try {
                            ...
                            if (testPattern) {
                                ...
                                const matchingFields = groupFields.filter(f => f.name && testPattern.test(f.name));
                                if (matchingFields.length >= 2) {
                                    helpers.logWithTimestamp(`Found valid rule pattern "${pattern}" for section ${targetSection} (${matchingFields.length} fields match)`, chalk.blue);
                                }
                            }
                        }
                        catch (patternError) {
     ...
     */
    extractPatternsFromField(field) {
        const patterns = [];
     ...
     */
    calculateAlignmentScore(deviations) {
        ...
        for (const field of fields) {
            ...
            try {
                ...
                if (results && results.length > 0 && results[0].section && results[0].section > 0) {
                    ...
                    if (results[0].subsection)
                        field.subsection = results[0].subsection;
     ...
     */
    async saveSectionData(sectionFields, outputPath) {
        try {
     ...
     */
    calculateAlignmentPercentage(sectionFields, referenceCounts) {
        // Get deviations
        const deviations = this.calculateDeviations(sectionFields, referenceCounts, this.deviationThreshold);
     ...
     */
    deduplicateFieldsById(sectionFields) {
     ...
     */
    countTotalFields(sectionFields) {
        let total = 0;
        for (const [section, fields] of Object.entries(sectionFields)) {
            total += fields.length;
        }
     ...
     */
    calculateContentRelevance(field, targetSection) {
        let relevance = 0;
        try {
            ...
            if (field) {
                ...
                if (valueStr.includes(`sec${targetSection}`) ||
     ...
     */
    calculatePageRelevance(field, targetSection, sourceSection) {
        if (!field.page)
            return 0;
        try {
     ...
     */
    directSectionBalance(sectionFields, referenceCounts) {
        const result = {
            corrections: 0,
            movesBySection: {}
        };
        ...
        const sectionZeroFields = sectionFields['0'] || [];
        if (oversizedSections.length === 0 && sectionZeroFields.length > 0 && undersizedSections.length > 0) {
            helpers.logWithTimestamp(`DIRECT BALANCE: All target sections undersized. Using section 0 as the sole source with ${sectionZeroFields.length} fields.`, chalk.red.bold // Differentiated log
            );
            oversizedSections = [{
                    section: 0,
                    actual: sectionZeroFields.length,
        ...
        helpers.logWithTimestamp(`DIRECT BALANCE: Found ${oversizedSections.length} oversized sections and ${undersizedSections.length} undersized sections`, chalk.red);
        ...
        for (const undersizedSection of undersizedSections) {
            ...
            let neededCount = Math.min(Math.abs(Math.ceil(undersizedSection.deviation)), 500 // Safety limit increased from 50 to 500 to move more fields at once
            );
            // Skip if we've already processed this section substantially
            if (processedSections.has(targetSection))
                continue;
            helpers.logWithTimestamp(`DIRECT BALANCE: Section ${targetSection} needs ${neededCount} more fields`, chalk.red);
            ...
            for (const oversizedSection of oversizedSections) {
                ...
                const sourceSection = oversizedSection.section;
                // Skip if we've already processed this section substantially
                if (processedSections.has(sourceSection))
                    continue;
                const sourceFields = sectionFields[sourceSection.toString()] || [];
                // Calculate how many we can take from this section
                const availableToTake = Math.min(Math.floor(oversizedSection.deviation), // Don't take more than the excess
                neededCount, 500 // Safety limit increased from 50 to 500
                );
                if (availableToTake <= 0)
                    continue;
                helpers.logWithTimestamp(`DIRECT BALANCE: Taking up to ${availableToTake} fields from section ${sourceSection} (${sourceFields.length} total) for section ${targetSection}`, chalk.red);
                // Find the lowest confidence fields in the source section
                const candidatesToMove = this.findLowConfidenceFields(sourceFields, availableToTake);
                if (candidatesToMove.length > 0) {
                    // Move the fields to the target section
                    this.moveFieldsToSection(sectionFields, candidatesToMove, targetSection, true);
                    ...
                    helpers.logWithTimestamp(`DIRECT BALANCE: Moved ${movedCount} fields from section ${sourceSection} to ${targetSection}`, chalk.green);
                }
            }
        }
        return result;
     ...
     */
    calculateTotalDeviation(deviations) {
     ...
     */
    normalizeReferenceCounts(referenceCounts) {
        const normalized = {};
...
export async function runConsolidatedSelfHealing(ruleEngine, sectionFields, referenceCounts, outputPath, deviationThreshold) {
