...
import { enhancedMultiDimensionalCategorization, extractSectionInfoFromName, identifySectionByPageWithConfidence, detectSectionFromFieldValue, sectionClassifications, refinedSectionPageRanges } from './fieldParsing.js';
...
export class ConsolidatedSelfHealingManager {
     ...
     */
    constructor(maxIterations = 5, deviationThreshold = 0.3) {
    ...
    async runSelfHealing(ruleEngine, sectionFields, referenceCounts, outputPath) {
        ...
        let remainingUnknown = [...unknownFields];
        ...
        if (unknownFields.length > 0) {
            const unknownResult = await this.processUnknownFields(ruleEngine, remainingUnknown, true, // Auto-update rules
            true // Log progress
            );
        ...
        if (initialDeviations.some(d => d.isSignificant)) {
            ...
            while (this.iteration < this.maxIterations) {
                ...
                // Log progress
                helpers.logWithTimestamp(`Applied ${stepResult.corrections} corrections, alignment score: ${stepResult.alignmentScore.toFixed(2)}%`, stepResult.corrections > 0 ? chalk.green : chalk.yellow);
        ...
        // Log final status
        helpers.logWithTimestamp(`Self-healing complete: ${success ? 'SUCCESS' : 'PARTIAL SUCCESS'} (${alignmentScore.toFixed(2)}% alignment)`, success ? chalk.green : chalk.yellow);
     ...
     */
    applyEnhancedCategorization(unknownFields, engine) {
