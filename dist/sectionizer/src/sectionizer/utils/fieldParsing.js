 ...
 */
import { subsectionPatterns, entryPatterns, sectionFieldPatterns, sectionEntryPrefixes, expectedFieldCounts } from './field-clusterer.js';
/**
 ...
 */
export function extractSectionInfo(fieldName, options = {}) {
    if (!fieldName)
        return null;
    ...
    if (sectionLetterMatch) {
        ...
        if (section === 21 && subsection === 'C') {
            ...
            if (textFieldMatch) {
                ...
                const mappedEntry = entryMapping[fieldIndex];
                if (mappedEntry) {
 ...
 * @returns Section information or null if no information could be extracted
 */
export function extractSectionInfoFromField(field) {
    if (!field)
        return null;
 ...
 */
export function getSpecialSectionPatterns(sectionNumber) {
...
export { identifySectionByPage, sectionClassifications, getSectionPageRange, refinedSectionPageRanges, identifySectionByPageWithConfidence, enhancedMultiDimensionalCategorization, detectSectionFromFieldValue, extractSectionInfoFromName, getNeighborFieldContext, initPageCategorization, enhancedSectionCategorization, updateFieldWithPageData } from '../../../utilities/page-categorization-bridge.js';
