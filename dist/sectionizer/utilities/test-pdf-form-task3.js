...
import { enhancedMultiDimensionalCategorization, groupFieldsBySection as groupFieldsBySectionBridge, sectionClassifications, getNeighborFieldContext, identifySectionByPageWithConfidence, refinedSectionPageRanges, 
// Import these patterns from the page-categorization-bridge file
sectionFieldPatterns } from './page-categorization-bridge';
        ...
        for (const file of contextFiles) {
            try {
                ...
                if (fileToSectionMapping[basename]) {
                    ...
                    contexts[sectionId] = contextData;
...
function collectFieldMetadata(field) {
    ...
    if (collectedMetadata.some(meta => meta.name === field.name))
        return;
...
function saveCollectedMetadata() {
    try {
        ...
        let combinedMetadata = [...collectedMetadata];
        if (fs.existsSync(metadataPath)) {
            try {
                ...
                if (Array.isArray(existingMetadata)) {
                    ...
                    const existingMetadataMap = new Map(existingMetadata.map(meta => [meta.name, meta]));
...
function getMetadataByName(fieldName) {
...
function updateFieldWithPageData(field, pdfDoc) {
    try {
        if (!field.name || !pdfDoc)
            return;
...
async function getFieldPageInfo(pdfDoc, fieldName) {
    try {
...
function detectSectionFromFieldValue(value) {
...
async function generateReports(validFields, unidentifiedFields) {
    try {
        console.log("\nGenerating reports...");
        const reportsDir = path.resolve(__dirname, "../reports");
        // Create directory if it doesn't exist
        ...
        for (let sectionId = 0; sectionId <= 30; sectionId++) {
            ...
            const pages = Array.from(new Set(sectionFields.filter(f => f.page).map(f => f.page)));
...
async function generatePageSummaryReport(validFields) {
    try {
        const reportsDir = path.resolve(__dirname, "../reports");
...
async function generateSectionDistributionReport(validFields, unidentifiedFields) {
    try {
        const reportsDir = path.resolve(__dirname, "../reports");
        // Get all strict pattern fields for sections 1 and 3
        const section1Fields = validFields.filter(field => field.name && sectionFieldPatterns[1] && sectionFieldPatterns[1].some(pattern => pattern.test(field.name)));
        const section3Fields = validFields.filter(field => field.name && sectionFieldPatterns[3] && sectionFieldPatterns[3].some(pattern => pattern.test(field.name)));
...
async function generateHierarchicalFieldOrganization(fields) {
...
async function generateHierarchicalReport(validFields) {
    try {
...
function validateSectionAssignments(fields) {
            ...
            try {
                ...
                try {
                    ...
                    if (field.name && field.name.includes('Sections1-6[0]')) {
                        const specialResult = categorizeSections1to6Field(field.name, field.label, field.value);
                    ...
                    const enhancedResult = pageCategorization.enhancedMultiDimensionalCategorization(field.name, field.label, field.page || 0, typeof field.value === 'string' ? field.value :
                        (field.value !== undefined ? String(field.value) : undefined), neighborFields);
                    if (enhancedResult) {
        ...
        try {
            ...
            validFields.forEach((field) => {
                try {
                    ...
                    if (!reportTree[sectionKey])
                        reportTree[sectionKey] = {};
                    // Level 2: Subsection
                    const subsectionId = field.subsection || 'general';
                    if (!reportTree[sectionKey][subsectionId])
                        reportTree[sectionKey][subsectionId] = {};
