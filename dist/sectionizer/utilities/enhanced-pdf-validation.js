...
function getMetadataByName(fieldName, extractedMetadata) {
...
function parseFieldNamePattern(fieldName) {
    try {
        if (!fieldName)
            return null;
...
const sectionPageRanges = refinedSectionPageRanges;
// Updated function to identify section by page with improved accuracy using the ranges
function identifySectionByPage(pageNum) {
...
function categorizeSections1to6Field(fieldName, fieldLabel, fieldValue) {
    if (!fieldName)
        return null;
...
async function main() {
    try {
        ...
        const pageMapPath = path.join(__dirname, "../reports/field-page-mapping.json");
        ...
        console.log("Phase 3: Performing enhanced multi-dimensional field categorization...");
        ...
        Object.keys(fieldLabels).forEach((fieldName) => {
            try {
                ...
                if (sectionFieldPatterns[1].some((pattern) => pattern.test(fieldName))) {
                ...
                if (sectionFieldPatterns[2].some((pattern) => pattern.test(fieldName))) {
                    fieldCategories[fieldName] = {
                        section: 2,
                ...
                if (sectionFieldPatterns[3].some((pattern) => pattern.test(fieldName))) {
                    fieldCategories[fieldName] = {
                        section: 3,
                ...
                if (sectionFieldPatterns[4].some((pattern) => pattern.test(fieldName)) ||
                ...
                if (sectionFieldPatterns[5].some((pattern) => pattern.test(fieldName))) {
                    fieldCategories[fieldName] = {
                        section: 5,
                ...
                if (sectionFieldPatterns[6].some((pattern) => pattern.test(fieldName))) {
                    fieldCategories[fieldName] = {
                        section: 6,
                ...
                if (sectionFieldPatterns[7].some((pattern) => pattern.test(fieldName))) {
                    fieldCategories[fieldName] = {
                        section: 7,
                ...
                if (sectionFieldPatterns[8].some((pattern) => pattern.test(fieldName))) {
                    fieldCategories[fieldName] = {
                        section: 8,
                ...
                if (sectionFieldPatterns[9].some((pattern) => pattern.test(fieldName))) {
                ...
                if (fieldName.includes('Sections1-6[0]')) {
                    const specialResult = categorizeSections1to6Field(fieldName, getFieldLabel(fieldName), getFieldValue(fieldName, fieldCategories));
                    if (specialResult) {
                ...
                const result = enhancedMultiDimensionalCategorization(fieldName, getFieldLabel(fieldName), pageMapping[fieldName]?.page || 0, typeof getFieldValue(fieldName, fieldCategories) === "string"
                    ...
                    : getFieldValue(fieldName, fieldCategories) !== undefined
                        ...
                        : undefined, [] // Nearby fields could be added here for better context
                );
                if (result) {
        ...
        for (const section of sectionSummary) {
            ...
            console.log(`  Section ${section.sectionId} (${section.sectionName}): ${total} fields, ${filled} filled (${percentage}%), ${highConf} high confidence`);
            ...
            if (section.subsections && section.subsections.length > 0 && section.sectionId === 9) {
                for (const subsection of section.subsections) {
                    ...
                    console.log(`    ${section.sectionId}.${subsection.subsectionId} (${subsectionName}): ${subTotal} fields, ${subFilled} filled (${subPercentage}%), ${subHighConf} high confidence`);
                }
            }
        }
        ...
        console.log(`- Fields with page data: ${fieldMetadata.filter((f) => f.page && f.page > 0).length}`);
        console.log(`- Fields without page data: ${fieldMetadata.filter((f) => !f.page || f.page === 0).length}`);
        console.log(`- Fields with labels: ${fieldMetadata.filter((f) => f.label).length}`);
        console.log(`- Fields with values: ${fieldMetadata.filter((f) => f.value !== undefined).length}`);
        console.log(`- Fields with max length: ${fieldMetadata.filter((f) => f.maxLength).length}`);
        console.log(`- Fields with options: ${fieldMetadata.filter((f) => f.options && f.options.length > 0).length}`);
        // console.log(
        ...
        console.log(`- Total categorized fields: ${Object.values(fieldCategories).length}`);
        // Count total subsections
        const totalSubsections = sectionSummary.reduce((total, section) => total + (section.subsections?.length || 0), 0);
        ...
        const fieldHierarchy = organizeFieldsHierarchically(fieldMetadata, fieldCategories);
        // Save the hierarchical organization
        const hierarchyPath = path.join(__dirname, "../reports/field-hierarchy.json");
        ...
        console.log("3. Explore the field-hierarchy.json for an organized view by page, value pattern, and regex");
 ...
 */
async function extractFieldMetadataWithPages(pdfPath) {
    ...
    if (DEBUG) {
        console.log(`Fields with page data from PdfService: ${fieldMetadata.filter((f) => f.page).length} / ${fieldMetadata.length}`);
    }
 ...
 */
function analyzePageDistribution(pageMapping) {
    ...
    for (const [fieldName, fieldData] of Object.entries(pageMapping)) {
        ...
        const enhancedResult = enhancedSectionCategorization(fieldName, fieldData.label, pageNumber, fieldData.value);
        if (enhancedResult &&
    ...
    return {
        ...
        fieldsWithPage: Object.values(pageMapping).filter((data) => data.page > 0)
            .length,
        fieldsWithoutPage: Object.values(pageMapping).filter((data) => data.page === 0).length,
 ...
 */
function detectSectionPages(fields, pageMapping) {
    const pages = fields
...
function organizeFieldsHierarchically(fieldMetadata, fieldCategories) {
    ...
    fieldMetadata.forEach((field) => {
        ...
        const fieldCount = Object.values(contextMapObj).reduce((count, fields) => count + fields.length, 0);
        ...
        const regexConfidenceSum = Object.values(regexPatterns).reduce((sum, pattern) => sum + pattern.confidence, 0);
 ...
 */
function createFieldRegexPattern(fieldName) {
 ...
 */
function findPotentialAlternativeSections(field, fieldCategories, undersizedSections) {
    const alternatives = [];
 ...
 */
function attachSpatialInformation(hierarchy) {
    // Analyze each page
    for (const [pageNumStr, pageData] of Object.entries(hierarchy)) {
        ...
        const fieldsWithCoordinates = [];
        // Collect fields from all patterns and regexes
        Object.values(pageData.fieldsByValuePattern).forEach((patternData) => {
            Object.values(patternData.fieldsByRegex).forEach((regexData) => {
        ...
        if (fieldsWithCoordinates.length < 3)
            continue;
...
// Add a function to validate section assignments and enforce strict rules
function validateSectionAssignments(fieldCategories) {
    ...
    Object.entries(fieldCategories).forEach(([fieldName, data]) => {
        ...
        if (data.section === 1 &&
            !sectionFieldPatterns[1].some((pattern) => pattern.test(fieldName))) {
            data.section = 0;
        ...
        if (data.section === 2 &&
            !sectionFieldPatterns[2].some((pattern) => pattern.test(fieldName))) {
            data.section = 0;
            data.sectionName = "Unidentified";
            data.confidence = 0.1;
        ...
        if (data.section === 3 &&
            !sectionFieldPatterns[3].some((pattern) => pattern.test(fieldName))) {
            data.section = 0;
            data.sectionName = "Unidentified";
            data.confidence = 0.1;
        ...
        if (data.section === 4 &&
            !sectionFieldPatterns[4].some((pattern) => pattern.test(fieldName)) &&
            !fieldName.includes("SSN[") &&
            !fieldName.toLowerCase().includes("ssn")) {
            data.section = 0;
            data.sectionName = "Unidentified";
        ...
        if (data.section === 6 &&
            !sectionFieldPatterns[6].some((pattern) => pattern.test(fieldName))) {
            data.section = 0;
            data.sectionName = "Unidentified";
            data.confidence = 0.1;
 ...
 */
function getFieldValue(fieldName, fieldData) {
