...
export const refinedSectionPageRanges = {
...
export function loadFieldPageMapping() {
    ...
    try {
        ...
        if (fs.existsSync(pageMapPath)) {
            ...
            console.warn("Field page mapping file not found. Run enhanced-pdf-validation.ts first.");
            return {};
        }
    }
    catch (error) {
 ...
 */
export function loadSectionStats() {
    ...
    try {
        ...
        if (fs.existsSync(statsPath)) {
            ...
            sectionStats = data;
            console.log(`Loaded section statistics with ${data.sectionStats?.length || 0} section entries`);
            return data;
        }
        else {
            console.warn("Section statistics file not found. Run enhanced-pdf-validation.ts first.");
            return {};
        }
    }
    catch (error) {
 ...
 */
export function getFieldPage(fieldName) {
    if (!fieldPageMapping) {
 ...
 */
export function extractSectionInfoFromName(fieldName) {
    if (!fieldName)
        return null;
    ...
    if (fieldName.includes("Sections1-6")) {
        ...
        if (fieldName.includes("form1[0].Sections1-6[0].DropDownList7[0]") ||
 ...
 */
export function detectSectionFromFieldValue(value) {
    if (!value)
        return null;
 ...
 */
export function identifySectionByField(fieldName, fieldLabel, pageNum) {
    ...
    for (const [sectionId, prefixes] of Object.entries(sectionNamePrefixes)) {
        for (const prefix of prefixes) {
            if ((fieldName && fieldName.includes(prefix)) ||
    ...
    if (fieldName) {
        if (fieldName.includes("Sections1-6[0].TextField11[0]") ||
        ...
        if (fieldName.includes("Sections1-6[0].TextField11[3]") ||
        ...
        if (fieldName.includes("Sections1-6[0].DropDownList7[0]") ||
    ...
    for (const [sectionIdStr, [startPage, endPage]] of Object.entries(refinedSectionPageRanges)) {
 ...
 */
export function identifySectionByPage(pageNum) {
 ...
 */
export function identifySectionByPageWithConfidence(pageNum) {
    ...
    for (const [sectionIdStr, [startPage, endPage]] of Object.entries(refinedSectionPageRanges)) {
        ...
        if (pageNum >= startPage && pageNum <= endPage) {
            ...
            const confidence = rangeSize === 1 ? 0.95 : 0.9 - Math.min(rangeSize, 10) / 20;
            return {
                section: sectionId,
 ...
 */
export function enhancedSectionCategorization(fieldName, fieldLabel, pageNum, fieldValue) {
    ...
    if (pageNum > 0) {
        ...
        if (pageSectionInfo !== null) {
            ...
            const overlappingPages = Object.entries(refinedSectionPageRanges).filter(([_, [start, end]]) => pageNum >= start && pageNum <= end).length;
    ...
    if (fieldName) {
        ...
        if (fieldName.includes("Sections1-6")) {
            ...
            else if (fieldName.includes("TextField11[3]") ||
                fieldName.includes("TextField11[4]") ||
                fieldName.includes("School6_State[0]") ||
                fieldName.includes("DropDownList1[0]")) {
    ...
    if (fieldValue !== undefined && fieldValue !== null && bestMatch) {
        ...
        if (valueStr.trim() !== "") {
            ...
            if (valueMatch && Array.isArray(valueMatch) && valueMatch.length > 1) {
                ...
                if (valueSectionNum === bestMatch.section) {
                    ...
                    if (bestMatch.confidence < 0.8) {
                        ...
                        if (pageNum > 0) {
                            const [start, end] = refinedSectionPageRanges[valueSectionNum] || [0, 0];
                            if (pageNum >= start && pageNum <= end) {
            ...
            if (bestMatch && bestMatch.section === 30) {
                ...
                const continuationMatch = valueStr.match(/continued|continuation|additional info|refer to section|see section/i);
    ...
    if (bestMatch && pageNum > 0) {
        ...
        if (pageSectionInfo !== null &&
            ...
            const [expectedStart, expectedEnd] = refinedSectionPageRanges[bestMatch.section] || [0, 0];
 ...
 */
export function enhancedCategorizeField(field) {
    if (!field || !field.name)
        return null;
    ...
    const sectionInfo = enhancedSectionCategorization(fieldName, fieldLabel, pageNum, fieldValue);
    if (!sectionInfo)
        return null;
...
export function loadFieldLabels() {
    ...
    try {
        ...
        if (fs.existsSync(labelsPath)) {
            ...
            console.warn("Field labels file not found. Some categorization confidence may be reduced.");
        }
    }
    catch (error) {
...
export function initPageCategorization() {
    loadFieldPageMapping();
    loadSectionStats();
    loadFieldLabels();
 ...
 */
export function updateFieldWithPageData(field) {
    if (!field || !field.name)
        return;
 ...
 */
export function createEmptyFieldPageMapping() {
    const pageMappingPath = path.join(__dirname, "../reports/field-page-mapping.json");
 ...
 * This is used by the section mapper to ensure there's at least a minimal structure
 */
export function createEmptySectionStats() {
    const statsPath = path.join(__dirname, "../reports/section-page-stats.json");
    if (!fs.existsSync(statsPath)) {
 ...
 */
export function getSectionPageRange(sectionId) {
    return refinedSectionPageRanges[sectionId] || [0, 0];
}
/**
 * Get all fields from a specific section
 */
export function getFieldsForSection(sectionId, metadata) {
    return metadata.filter((field) => {
        const result = enhancedSectionCategorization(field.name, field.label, field.page || 0, field.value);
 ...
 */
export function getFieldPattern(fieldName) {
...
// Define comprehensive section classification mappings
export const sectionClassifications = [
    {
        sectionId: 1,
        name: "Full Name",
        pageRanges: [[5, 5]],
...
export const contextualFieldGroups = {
    // Section 1 - Full Name
 ...
 */
export function enhancedMultiDimensionalCategorization(fieldName, fieldLabel, pageNum, fieldValue, neighborFields = []) {
    ...
    for (const classification of sectionClassifications) {
        ...
        if (pageNum > 0) {
            ...
            const pageMatch = classification.pageRanges.some(([start, end]) => pageNum >= start && pageNum <= end);
            if (pageMatch) {
        ...
        if (fieldName) {
            ...
            const nameMatch = classification.fieldPathPatterns.some((pattern) => pattern.test(fieldName));
            if (nameMatch) {
        ...
        if (fieldValue && typeof fieldValue === "string" && fieldValue.length > 0) {
            ...
            const valueMatch = classification.valueKeywords.some((keyword) => fieldValue.toLowerCase().includes(keyword.toLowerCase()));
            if (valueMatch) {
                score += 1;
                matches++;
            }
        ...
        if (fieldName) {
            ...
            const contextMatch = classification.contextKeys.some((contextKey) => fieldName.includes(contextKey));
            if (contextMatch) {
                score += 1;
                matches++;
            }
        ...
        if (neighborFields.length > 0) {
            ...
            const neighborMatches = neighborFields.filter((neighbor) => classification.neighborFields.some((expectedNeighbor) => neighbor.toLowerCase().includes(expectedNeighbor.toLowerCase()))).length;
            // Calculate a proportional score based on matches
            if (neighborMatches > 0) {
                const proportionalScore = Math.min(1, neighborMatches / Math.min(5, classification.neighborFields.length));
        ...
        const confidence = 0.4 * classification.confidence +
            0.4 * matchProportion +
            0.2 * factorMatch;
 ...
 */
export function getNeighborFieldContext(fieldName, allFields) {
    ...
    const similarFields = allFields.filter((f) => {
        ...
        if (normalizedF.length > 5 && normalizedName.length > 5) {
            const prefix = normalizedF.substring(0, Math.min(normalizedF.lastIndexOf("["), normalizedF.lastIndexOf(".")));
            return normalizedName.startsWith(prefix);
        }
        return false;
    });
    // Find context group this field might belong to
    const contextGroup = Object.entries(contextualFieldGroups).find(([_, fields]) => fields.some((field) => fieldName.toLowerCase().includes(field.toLowerCase())))?.[0];
    ...
    if (contextGroup) {
        ...
        return allFields.filter((f) => groupFields.some((field) => f.toLowerCase().includes(field.toLowerCase())));
    }
 ...
 */
export function getComprehensiveFieldMetadata(fieldName, allFields, fieldValueGetter, fieldLabelGetter) {
    // Get page number
    ...
    const categorization = enhancedMultiDimensionalCategorization(fieldName, label, page, value ? String(value) : undefined, neighborFields);
    return {
        name: fieldName,
        page,
 ...
 */
export function groupFieldsBySection(fieldNames, fieldValueGetter, fieldLabelGetter) {
    const sections = {};
    // Initialize sections
    for (const classification of sectionClassifications) {
    ...
    for (const fieldName of fieldNames) {
        const metadata = getComprehensiveFieldMetadata(fieldName, fieldNames, fieldValueGetter, fieldLabelGetter);
