...
const SF86_SECTIONS: Record<string, SF86SectionConfig> = {
...
const metadataData = metadataObj as unknown as PdfMetadata | undefined;      if (metadataData !== undefined) {
...
const loadSearchValuesFromFile = async (filePath: string): Promise<string[]> => {
  try {
...
const getSF86SectionConfig = (sectionName: string): SF86SectionConfig | null => {
...
const parsePageRange = (pageRangeStr: string): [number, number] | null => {
...
const extractFormFields = async (
  pdfDocument: pdfjsLib.PDFDocumentProxy,
  sourceDescription: string
): Promise<{ fields: FormFieldData[]; summary: { total_fields: number; fields_with_values: number; field_types: Record<string, number> } }> => {
...
const performValidationSearch = (
  output: PdfResultData,
  searchValues: string[]
): ValidationResult[] => {
...
const processSingleSource = async (
  ...
  includeFormFields: boolean = false,
  validationMode: boolean = false,
  searchValues: string[] = [],
  sf86Info?: { section: string; page_range: string; auto_detected: boolean; pages_analyzed: number[] }
...
export const handleReadPdfFunc = async (
  ...
  let sf86Info: { section: string; page_range: string; auto_detected: boolean; pages_analyzed: number[] } | undefined;
...
export const readPdfToolDefinition: ToolDefinition = {
  name: 'read_pdf',
  description:
    'Enhanced PDF reader with SF-86 section-aware validation capabilities. Features: form field extraction, batch search from text files, automatic SF-86 section page detection, absolute path support in validation mode. Supports all SF-86 sections with automatic page range detection.',
