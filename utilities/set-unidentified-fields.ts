...
interface UnidentifiedFieldsData {
  count: number;
  fields: UnidentifiedField[];
}
...
async function main(): Promise<void> {
  try {
    ...
    const unidentifiedFieldsData: UnidentifiedFieldsData = JSON.parse(
      fs.readFileSync(UNIDENTIFIED_FIELDS_PATH, 'utf-8')
    );
